...
void main() {
  ...
  group('MERZOX seed goldens - Arabic, 375x812, Windows canonical', () {
    setUpAll(() async {
      SharedPreferences.setMockInitialValues(<String, Object>{});
    ...
    testWidgets('store preview renders its Arabic loaded-state baseline', (
      ...
      final Future<BusinessProfileState> storefrontReady = storefrontBloc.stream
          .firstWhere(
    ...
    testWidgets('cart renders its Arabic guest-state baseline', (
      ...
      final HomeBloc homeBloc = HomeBloc(apiService: _OfflineHomeApiService());
      ...
      await pumpMerzoxGoldenPage(
        ...
        BlocProvider<HomeBloc>.value(
          value: homeBloc,
          child: withMerzoxGoldenDeviceInsets(const HomeScreen(isGuest: true)),
        ),
      );
    ...
    testWidgets('order tracking renders its Arabic placed-state baseline', (
      ...
      final Future<OrderTrackingState> ready = trackingBloc.stream.firstWhere(
        (OrderTrackingState state) => state.status == OrderTrackingStatus.ready,
      );
    ...
    testWidgets('about us renders its Arabic loaded-state baseline', (
      ...
      final AboutUsBloc aboutBloc = AboutUsBloc(apiService: _SeedAboutUsApi());
    ...
    testWidgets('customer profile renders its Arabic guest baseline', (
      ...
      await bloc.stream.firstWhere((HomeState state) => state.selectedTab == 4);
    ...
    testWidgets('customer profile renders its Arabic signed-in baseline', (
      ...
      await pumpMerzoxGoldenPage(
        ...
        BlocProvider<HomeBloc>.value(
          value: bloc,
          child: withMerzoxGoldenDeviceInsets(const HomeScreen(isGuest: false)),
        ),
      );
    ...
    testWidgets('profile form renders its Arabic baseline', (
      ...
      await pumpMerzoxGoldenPage(
        ...
        BlocProvider<ProfileEditBloc>.value(
          value: bloc,
          child: withMerzoxGoldenDeviceInsets(ProfileEditPage(apiService: api)),
        ),
      );
    ...
    testWidgets('store settings description renders its Arabic baseline', (
      ...
      await expectMerzoxSeedGolden('store_settings_description_ar_375x812.png');
    });
    ...
    testWidgets('store settings social renders its Arabic baseline', (
      ...
      expect(find.text('قم بإدخال يوزر الانستغرام او الرابط'), findsOneWidget);
    ...
    testWidgets('the stores tab renders its Arabic baseline', (
      ...
      await bloc.stream.firstWhere((HomeState state) => state.selectedTab == 2);
    ...
    testWidgets('notifications list renders its Arabic baseline', (
      ...
      final Future<NotificationsState> ready = bloc.stream.firstWhere(
        (NotificationsState state) => state.status == NotificationsStatus.ready,
      );
    ...
    Future<OrdersBloc> customerOrders(ApiService api, OrdersGroup group) async {
      _useAuthenticatedCustomerSession();
    ...
    testWidgets('the merchant order status menu renders its Arabic baseline', (
      ...
      await expectMerzoxSeedGolden('merchant_order_status_menu_ar_375x812.png');
    });
    ...
    testWidgets('the notified merchant order renders its Arabic baseline', (
      ...
      tester
          ...
          .showSnackBar(
            merchantOrderNoticeSnackBar('merchantOrder.notificationSent'.tr()),
          );
      await settleMerzoxGoldenFrames(tester);
    ...
    testWidgets('merchant enrolment renders its Arabic second-step baseline', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('order tracking renders its Arabic on-the-way baseline', (
      ...
      await expectMerzoxSeedGolden('order_tracking_on_the_way_ar_375x812.png');
    });
    ...
    Future<void> pumpChat(
      ...
      final ChatBloc bloc = ChatBloc(
        apiService: _SeedChatApi(asCustomer: asCustomer, lineCount: lineCount),
        conversationId: 'c-seed',
      );
      _closeOnTearDown(bloc);
